...
-> file { 'C:/install_tentacle.bat':
  ...
  content => @(EOT)
    ...
    "C:\Program Files\Octopus Deploy\Tentacle\Tentacle.exe" register-with --instance "Tentacle" --server "http://localhost" --name "LocalTentacle" --comms-style "TentacleActive" --server-comms-port "10943" --username "admin" --password "Password01!" --space "Default" --environment "Dev" --environment "Test" --environment "Prod" --role "web" --policy "Default Machine Policy"
