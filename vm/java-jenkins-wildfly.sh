...
# Java Jenkins Wildfly Builtin Feed
...
export CUCUMBER_TAGS="(not @browserspecific or @chrome) and (not @destinationspecific or @wildfly) and (not @repositoryspecific or @octo-built-in-feed) and (not @applicationspecific or @java) and (not @sourcespecific or @git)"
...
# Open test app
./scripts/linux/open-wildflydevwebapp.sh
./scripts/linux/open-wildflytestwebapp.sh

# Add desktop shortcuts
sudo -Hu vagrant DBUS_SESSION_BUS_ADDRESS=unix:path=/run/user/1000/bus dconf write /org/gnome/shell/favorite-apps \
  "['Passwords.desktop', 'Guides.desktop', 'Octopus.desktop', 'Jenkins.desktop', 'Random Quotes Dev.desktop', 'Random Quotes Test.desktop', 'Wildfly.desktop', 'gnome-terminal.desktop']"
